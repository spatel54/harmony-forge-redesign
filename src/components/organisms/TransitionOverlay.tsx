...
/** Duration in ms — must match the setTimeout in the calling view */
...
/**
 * Counts from 0 to 100 over `duration` ms while `visible` is true.
 * Resets to 0 whenever `visible` flips to false.
 */
...
/**
 * BookLoader — exact port of the original styled-components snippet.
 * Styles are injected inline via <style> to guarantee they are present
 * regardless of Tailwind build pipeline or globals.css load order.
 */
function BookLoader() {
  return (
    <>
...
// ─── Music Note Loader (generating variant) ──────────────────────────────────
...
/** Single music note SVG — inherits `currentColor` for fill */
function NoteIcon() {
  return (
    <svg
...
/**
 * MusicNoteLoader — three bouncing music notes with staggered delays.
 * Styles injected via <style> so !important overrides the prefers-reduced-motion
 * global kill in globals.css.
 */
  ...
  return (
    <>
      ...
      <div role="img" aria-hidden="true" className="hfn">
        <span className="hfn-icon"><NoteIcon /></span>
        <span className="hfn-icon"><NoteIcon /></span>
        <span className="hfn-icon"><NoteIcon /></span>
      </div>
    </>
 ...
 *
 * Variants:
 *  - "parsing"    → Upload → Document   (3-D book page-turning)
 *  - "generating" → Document → Sandbox  (bouncing music notes)
 *
 * Both variants show a live percentage counter and a deterministic
 * progress bar driven by usePercentageCounter.
 */
export function TransitionOverlay({
  ...
  return (
    ...
    >
      {/* Variant animation */}
      {variant === "parsing" ? <BookLoader /> : <MusicNoteLoader />}
